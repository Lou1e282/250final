...
client = mqtt.Client()
client.username_pw_set("louieshe", "aio_CFnk74c44RttQTiFy9UUcHvnpaVS")
...
while True: 
    try: 
        ...
        if alarm_flag == 0 and len(variances) == 15:
            avg_variance = sum(variances) / 15
            if avg_variance > VAR_THRESHOLD:
                alarm_flag = 2  # theft (overrides crash)
